...
///
/// It will be visible in analytics, in the viewer's about-menu, and with `platform --version`.
///
...
///
/// It will be visible in analytics, in the viewer's about-menu, and with `pl --version`.
///
/// To do so accurately may incur unnecessary recompiles, so only turn this on if you really need it.
...
pub enum Environment {
    ...
    /// github-hosted tgz.
    CondaBuild,
    /// Are we a developer running inside the workspace?
    DeveloperInWorkspace,
    /// We are not on CI, and not in the Rerun workspace.
    ///
    /// This is _most likely_ a Rerun user who is compiling a `PL_` crate
    /// because they depend on it either directly or indirectly in their `Cargo.toml`,
    /// or they running `cargo install platform-cli` or other tool that depend on a `PL_` crate.
    ///
...
///
/// Use this crate together with the `PL_build_info` crate.
pub fn export_build_info_vars_for_crate(crate_name: &str) {
    ...
    let export_git_info = match environment {
        ...
        // We shouldn't show the users git hash/branch in the platform viewer.
    ...
    if export_datetime {
        set_env("PL_BUILD_DATETIME", &date_time());
    ...
    if export_git_info {
        ...
        set_env("PL_BUILD_GIT_HASH", "");
        set_env("PL_BUILD_GIT_BRANCH", "");
    }

    // Stuff that doesn't change, so doesn't need rebuilding:
    {
        // target triple
        set_env("PL_BUILD_TARGET_TRIPLE", &std::env::var("TARGET").unwrap());

        // rust version
        let (rustc, llvm) = rust_llvm_versions().unwrap_or_default();
        set_env("PL_BUILD_RUSTC_VERSION", &rustc);
        set_env("PL_BUILD_LLVM_VERSION", &llvm);
        ...
        if is_on_ci() {
            ...
            set_env("PL_BUILD_IS_IN_RERUN_WORKSPACE", "no");
        } else {
            set_env(
                "PL_BUILD_IS_IN_RERUN_WORKSPACE",
